...
try:
    ...
    load_dotenv = None
from core.paths import ARTIFACT_OUTPUT_DIR, ROOT_DIR, SAMPLE_DATA_DIR
...
DEFAULT_OUTPUT = ARTIFACT_OUTPUT_DIR / "similar_patent_analysis.json"
...
def infer_technical_analysis(target: dict[str, Any], detail: dict[str, Any], signals: dict[str, float], keywords: list[str]) -> dict[str, str]:
    ...
    return {
        ...
        "maintenance_implication": (
            ...
            "따라서 유지 판단에서는 단순 유사도보다 독립항의 필수 구성요소가 실제로 겹치는지와 회피설계 가능성을 우선 확인해야 합니다."
        ),
    }
...
def build_interpretation(ecosystem: dict[str, Any], target_position: dict[str, Any], top_comparisons: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    maintenance_signal = "긍정" if active_ratio >= 0.7 else "재검토" if active_ratio <= 0.4 else "중립"
    ...
    decision_comment = (
        ...
        "최종 유지 판단에서는 상위 비교 특허의 청구항과 대상 특허의 보이스 스타일 분석, 제작 그룹 구성, 품질 검수·보정, 스마트컨트랙트 연계 요소가 동시에 포섭되는지 따져봐야 합니다."
    )
...
{{
  "summary": "2문장. 왜 중요한 비교 대상인지와 유지 판단 시사점",
  ...
  "maintenance_implication": "대상 특허 유지/포기 판단에 주는 실무적 시사점"
}}"""
...
def fallback_decision_comment(ecosystem: dict[str, Any], top_comparisons: list[dict[str, Any]]) -> str:
    ...
    implications = [
        item.get("comparison", {}).get("technical_analysis", {}).get("maintenance_implication", "")
        ...
        comparison_digest.append(
            {
                ...
                "maintenance_implication": analysis.get("maintenance_implication"),
            }
        )
...
- 유지 판단을 쓰더라도 반드시 상위 비교 특허의 청구항/기술 차이에 근거해 표현
- 마크다운 금지"""
    comment = call_llm(prompt, max_tokens=800)
    if comment:
        interpretation["decision_comment"] = remove_generic_maintenance_sentence(comment)
        interpretation["decision_comment_method"] = "llm"
    else:
        interpretation["decision_comment"] = fallback_decision_comment(ecosystem, top_comparisons)
        interpretation["decision_comment_method"] = "rule"
...
def analyze_similar_patents(
    ...
    llm_used = (
        ...
        or interpretation.get("decision_comment_method") == "llm"
    )
