...
class PatentValuationOptionsRequest(BaseModel):
...
class PatentValuationRequest(BaseModel):
    """개발/디버그용 특허 가치평가 보고서 요청입니다."""

    patent_data: dict[str, Any] = Field(..., description="특허 입력 JSON")
    options: PatentValuationOptionsRequest = Field(default_factory=PatentValuationOptionsRequest)


class PatentValuationResponse(BaseModel):
    """특허 가치평가 보고서 workflow 응답입니다."""
...
class PdfMetadataToolRequest(BaseModel):
    ...
    source_pdf: str = Field(..., description="data/samples/patent_documents 아래 파일명 또는 서버 로컬 경로")
