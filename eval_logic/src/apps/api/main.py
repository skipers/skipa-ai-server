"""SKIPA 특허 가치평가 보고서 AI Backend API입니다."""
...
from apps.api.job_store import create_job, get_job, set_completed, set_failed, set_running
from apps.api.schemas import (
    BusinessRagToolRequest,
    PatentValuationOptionsRequest,
    PatentValuationRequest,
    PatentValuationResponse,
    PatentReportRequest,
    PatentToolRequest,
    ReportJobResponse,
...
app = FastAPI(
    ...
    description=(
        "특허 가치 평가, 추가 자료 수집, agentic workflow 기반 "
        "가치평가 보고서 생성 API"
    ),
    version="0.2.0",
)
...
def _save_api_test_report_result(job_id: str, result: dict[str, Any]) -> str:
    path = API_TEST_REPORT_OUTPUT_DIR / safe_report_filename_from_result(result)
    saved = {
        "job_id": job_id,
...
def _run_workflow_for_json(patent: dict[str, Any]) -> dict[str, Any]:
    ...
    return PatentValuationWorkflow(options).run(patent)
...
def _run_workflow_for_pdf(source_pdf: str) -> dict[str, Any]:
    ...
    result = PatentValuationWorkflow(options).run({"source_pdf": source_pdf})
...
def create_patent_report_from_json(request: PatentReportRequest) -> dict[str, Any]:
    ...
    job_id = create_job("patent-valuation-json", payload)
    ...
    return {
        ...
        "message": "특허 가치평가 보고서 생성 Job이 처리되었습니다.",
...
@app.post(
    "/api/v1/reports/patent-valuation/from-json-file",
    response_model=ReportJobResponse,
    tags=["reports"],
)
def create_patent_report_from_json_file(file: UploadFile = File(...)) -> dict[str, Any]:
    """특허 JSON 파일 업로드로 가치평가 보고서 Job을 생성합니다.

    ``data/samples/input`` 아래 샘플처럼 특허 JSON 자체를 업로드해도 되고,
    ...
    return {
        "job_id": job_id,
        "status": job.get("status", "unknown"),
        "message": "JSON 파일 기반 특허 가치평가 보고서 생성 Job이 처리되었습니다.",
        "status_url": status_url,
        "result_url": result_url,
        "input_path": input_path,
        "output_path": (result.get("artifacts") or {}).get("report_output_path"),
...
@app.post(
    "/api/v1/reports/patent-valuation/from-pdf",
    response_model=ReportJobResponse,
    tags=["reports"],
)
def create_patent_report_from_pdf(file: UploadFile = File(...)) -> dict[str, Any]:
    """특허 PDF 파일 업로드로 가치평가 보고서 Job을 생성합니다."""
    dest = _save_uploaded_pdf(file, RUNTIME_UPLOAD_DIR)

    job_id = create_job("patent-valuation-pdf", {"source_pdf": str(dest)})
    ...
    return {
        "job_id": job_id,
        "status": job.get("status", "unknown"),
        "message": "PDF 기반 특허 가치평가 보고서 생성 Job이 처리되었습니다.",
        "status_url": status_url,
        "result_url": result_url,
...
def get_report_job_status(job_id: str) -> dict[str, Any]:
    ...
    progress = min(int(len(node_trace) / 7 * 100), 100) if node_trace else 0
    return {
        "job_id": job_id,
        "status": job.get("status", "unknown"),
...
@app.post("/api/v1/dev/patent-valuation/evaluate", response_model=PatentValuationResponse, tags=["dev"])
def dev_evaluate_patent_valuation(request: PatentValuationRequest) -> dict[str, Any]:
    """옵션을 직접 제어하는 개발/디버그용 동기 평가 API입니다."""
    try:
        option_dict = _model_to_dict(request.options)
        options = PatentValuationWorkflowOptions.from_dict(option_dict)
        return PatentValuationWorkflow(options).run(normalize_patent_input(request.patent_data))
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc)) from exc


@app.post(
    "/api/v1/dev/patent-valuation/evaluate-sample/{sample_name}",
    response_model=PatentValuationResponse,
    tags=["dev"],
)
def dev_evaluate_sample_patent(
    sample_name: str,
    options: PatentValuationOptionsRequest | None = None,
) -> dict[str, Any]:
    """``data/samples/input`` 아래 샘플 JSON 파일을 옵션과 함께 실행합니다."""
    ...
    return PatentValuationWorkflow(PatentValuationWorkflowOptions.from_dict(option_dict)).run(
